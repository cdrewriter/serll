...
import { Paper, Box, Container, Grid, SvgIcon } from '@material-ui/core';
import Breadcrumbs from '../../components/breadcrumbs/Breadcrumbs';
import PriceCategories from '../../components/sidebars/PriceCategoriess';
...
const ServiceDetail = () => {
  ...
  const result = useGraphQL({
    ...
    operation: {
      query: /* GraphQL */ `
        query PostDetail($postWhere: ItemServiceWhereInput) {
          allItemServices(where: $postWhere) {
            id
            name
            image {
              publicUrl
  ...
  if (cacheValue && cacheValue.data) {
    const { allItemServices, allItemServiceCategories } = cacheValue.data;
    if (!allItemServices.length) {
      // When post is not found
      return <>Not fou1nd</>;
    }

    const post = allItemServices[0];
    return (
      <>
        <Container fixed>
          <Breadcrumbs
        ...
        <Container maxWidth={false}>
          <Grid container>
            <Grid item xs={3}>
              <PriceCategories priceCategories={allItemServiceCategories} activeKey={slug} />
            </Grid>
            <Grid item xs={9}>
              <Paper elevation={0}>
                <Tabll data={allItemServices} />
              </Paper>
            </Grid>
          </Grid>
        </Container>
