...
function ListItemLink(props) {
  return <ListItem button component="a" className={props.className}  {...props} />;
}
...
const PriceCategories = ({ priceCategories, activeKey }) => {
  ...
  for (let i = 0; i < (priceCategories ? priceCategories.length : 0); ++i) {
    ...
    items.push(
      <ListItemLink key={cat.id} className={`${active}`} href={`/cars/${cat.slug}`}>
        {active ? cat.name : <ListItemText primary={cat.name} />}
      </ListItemLink>
    );
  }
  return (
    <ListGroup component="ul" className="cars-categories" as="ul">
      {items}
    </ListGroup>
  );
};
